...
2. 이후 관찰 윈도우(`rope_stuck_window_frames`) 동안 다음 반등이나 release 회복이 이어지는지 본다.
3. 윈도우 안에 정상 연속 신호(다음 rebound 후보 또는 release 회복)가 오면 보정 없이 그대로 둔다.
4. 윈도우가 끝날 때까지 그 둘이 없고 접지 정체 + 시도 흔적만 남았으면 줄걸림으로 판단하고 `-1` 보정한다.

줄걸림 판정은 반드시 **윈도우 종료 시점에 한 번만** 내린다.
연속 점프에서는 다음 반등까지 정상적으로 십수 프레임의 접지가 존재하므로,
윈도우 중간에 성급하게 판정하면 낮은 점프의 정상 count가 전부 `-1`로 상쇄되는 문제가 생긴다.
