...
2. 이후 관찰 윈도우(`rope_stuck_window_frames`) 동안 반대발 전환이나 dual-air 회복이 이어지는지 본다.
3. 윈도우 안에 그 둘 중 하나라도 오면 보정 없이 그대로 둔다.
4. 윈도우가 끝날 때까지 같은 발에 정체되어 있으면 줄걸림으로 판단하고 `-1` 보정한다.

줄걸림 판정은 반드시 **윈도우 종료 시점에 한 번만** 내린다.
정상 스텝에서도 착지 직후 몇 프레임은 같은 지지발이 유지되므로,
윈도우 중간에 성급하게 판정하면 정상 스텝 count가 전부 `-1`로 상쇄되는 문제가 생긴다.
